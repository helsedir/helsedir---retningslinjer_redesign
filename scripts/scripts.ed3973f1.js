...
function responsiveTables() {
    loopTables(document), $(".recommendationheader, .aboutguidelineicon, .rationallink, .practicaladvicelink, .picosummarylink, .resp-accordion").click(function() {
        var a;
        $(this).hasClass("recommendationheader") ? a = $(this).closest(".Recommendation").find(".textContainer") : $(this).hasClass("aboutguidelineicon") ? a = $(this).closest(".pageheader").find(".aboutexpand") : $(this).hasClass("picosummarylink") && (a = $(document).find(".picosummary")), window.setTimeout(function() {
            loopTables(a)
        }, 100)
    })
}

function loopTables(a) {
    $(a).find("table").each(function() {
        var a = $(this).width(),
            b = $(this).parent().width();
        b > 0 && a > b && (addDataHeaders(this), $(this).addClass("breakTable"))
    })
}

function addDataHeaders(a) {
    var b = [];
    $(a).find("tr").each(function(a) {
        0 == a && $(this).addClass("hidden"), $(this).find("td").each(function(c) {
            0 == a ? b.push(this.innerText) : $(this).attr("data-th", b[c])
        })
    })
}
...
angular.module("guidelinePreviewApp", ["ngRoute", "ngResource", "toastr", "LocalStorageModule", "ui.bootstrap"]).config(["$httpProvider", function(a) {
...
}]), angular.module("guidelinePreviewApp").factory("authInterceptorService", ["$q", "$location", "localStorageService", function(a, b, c) {
    var d = {},
        e = function(a) {
            a.headers = a.headers || {};
        ...
        f = function(c) {
            return 401 === c.status && b.path("/login"), a.reject(c)
        };
    return d.request = e, d.responseError = f, d
}]);
var apiUrl = "https://data.helsedirektoratet.no/api/v1/";
angular.module("guidelinePreviewApp").factory("Guideline", ["$resource", function(a) {
    ...
    }]).factory("Section", ["$resource", function(a) {
    ...
    }]).factory("Recommendation", ["$resource", function(a) {
    ...
    }]).factory("Author", ["$resource", function(a) {
        return a(apiUrl + "authors/:_id", {}, {
            update: {
                method: "PUT"
            }
        })
    }]).factory("Pico", ["$resource", function(a) {
    ...
    }]).factory("PicoCode", ["$resource", function(a) {
    ...
    }]).factory("TaxonomyCode", ["$resource", function(a) {
        return a(apiUrl + "taxonomycodes/:_id", {}, {
            update: {
                method: "PUT"
            }
        })
    }]).factory("EmrInfo", ["$resource", function(a) {
        return a(apiUrl + "emrinfos/:_id", {}, {
            update: {
                method: "PUT"
            }
        })
    }]).factory("KeyInfo", ["$resource", function(a) {
        return a(apiUrl + "keyinfos/:_id", {}, {
            update: {
                method: "PUT"
            }
        })
    }]).factory("PicoContinousOutcome", ["$resource", function(a) {
        return a(apiUrl + "picocontinousoutcomes/:_id", {}, {
            update: {
                method: "PUT"
            }
        })
    }]).factory("PicoOutcome", ["$resource", function(a) {
        return a(apiUrl + "picooutcomes/:_id", {}, {
            update: {
                method: "PUT"
            }
        })
    }]).factory("Reference", ["$resource", function(a) {
    ...
    }(jQuery), $(runJqueryUIStuff()), $(runJqueryUIRecommendationStuff()), angular.module("guidelinePreviewApp").controller("LoginCtrl", ["$scope", "$location", "authService", "$rootScope", function(a, b, c, d) {
    ...
    }]), angular.module("guidelinePreviewApp").controller("GuidelineCtrl", ["Guideline", "toastr", "$scope", "$routeParams", "$location", "ErrorService", function(a, b, c, d, e, f) {
        var g = d.guidelineId,
            h = !1;
        h = e.search().preview, a.get({
            _id: g,
            preview: h
    ...
    }]), angular.module("guidelinePreviewApp").controller("SectionCtrl", ["Section", "Guideline", "toastr", "$scope", "$routeParams", "$location", "ErrorService", function(a, b, c, d, e, f, g) {
        var h = e.guidelineId,
            i = e.sectionId,
            j = !1;
        j = f.search().preview, b.get({
            _id: h,
            preview: j
        }).$promise.then(function(a) {
            d.guideline = a
    ...
    }]), angular.module("guidelinePreviewApp").controller("RecommendationCtrl", ["$scope", "Recommendation", "Guideline", "toastr", "$routeParams", "$location", "ErrorService", "$rootScope", function(a, b, c, d, e, f, g, h) {
        h.recommendation = !0;
        var i = e.recommendationId,
            j = e.guidelineId,
            k = !1;
        k = f.search().preview, b.get({
            _id: i,
            preview: k
    ...
    }]), angular.module("guidelinePreviewApp").controller("MainCtrl", ["$scope", "Guideline", "toastr", "ErrorService", function(a, b, c, d) {
        a.refineSearch = 0, b.query().$promise.then(function(b) {
            a.guidelines = b
        }, function(a) {
            d.handleError(a)
        })
            ...
            link: function(a, b) {
                var c = function() {
                    b.easyResponsiveTabs({
                        ...
                        slideSpeed: 200,
                        activate: function() {
                            $(this).closest("ul").addClass("open"), loopTables($(this).closest(".tabs").find(".resp-tabs-container"))
                        }
                    })
                };
                a.$watch(b, c)
                ...
                a.showAlert = !0, a.closeAlert = function() {
                    ...
                    switch (a.publishedStage) {
                        ...
                        case 2:
                            ...
                            break;
                        case 4:
                            a.publishedStageText = "intern høring"
                    }
                })
            }
        }
    ...
    }), angular.module("guidelinePreviewApp").service("ErrorService", ["toastr", function(a) {
    ...
    }]), angular.module("guidelinePreviewApp").factory("authService", ["$http", "$q", "localStorageService", function(a, b, c) {
        var d = "https://data.helsedirektoratet.no/",
            e = {},
            f = {
                isAuth: !1,
                userName: ""
    ...
    }]), angular.module("guidelinePreviewApp").controller("PrintCtrl", ["Guideline", "Section", "Recommendation", "$routeParams", "ErrorService", "$location", "$scope", "$rootScope", function(a, b, c, d, e, f, g, h) {
        h.recommendation = !0;
        var i = d.guidelineId,
            j = !1;
    ...
    }), angular.module("guidelinePreviewApp").directive("printRecommendation", ["Recommendation", "ErrorService", function(a, b) {
        return {
